...
namespace AsteroidsBL
{
    public class SortOrderBuilder : ISortOrderBuilder
    {
        ...
        public SortOrderBuilder(ParallelQuery<Asteroid> query)
        {
            this.query = query;
        }
        ...
        IFilteredSortedSet<IAsteroid> ISortOrderBuilder<IAsteroid>.GetFilteredSortedSet()
        {
            if(first)
            {
                DefOrderBy(r => r.DistanceSunInAU, false);
            }
            return new FilteredSortedSet(orderedQuery);
        }
    }
}
